  ...
  controlButtons.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (command && wsClient.ws && wsClient.ws.readyState === WebSocket.OPEN) {
        wsClient.sendMovement(command, duration, APP_STATE.currentSpeed);
...
function handleKeyboard(e) {
  ...
  switch (e.key.toLowerCase()) {
    ...
    case "arrowup":
      e.preventDefault();
      wsClient.sendMovement(COMMANDS.FORWARD, duration, APP_STATE.currentSpeed);
      break;
    case "s":
    case "arrowdown":
      e.preventDefault();
      wsClient.sendMovement(COMMANDS.BACKWARD, duration, APP_STATE.currentSpeed);
      break;
    case "a":
    case "arrowleft":
      e.preventDefault();
      wsClient.sendMovement(COMMANDS.LEFT, duration, APP_STATE.currentSpeed);
      break;
    case "d":
    case "arrowright":
      e.preventDefault();
      wsClient.sendMovement(COMMANDS.RIGHT, duration, APP_STATE.currentSpeed);
      break;
    case " ":
    case "escape":
      e.preventDefault();
...
async function executeCustomSequence() {
  ...
  try {
    ...
    for (let i = 0; i < SEQUENCE_STATE.customSequence.length; i++) {
      ...
      wsClient.sendMovement(movement.command, movement.duration, movement.speed || APP_STATE.currentSpeed);
...
function loadPredefinedDemo(demoType) {
  const predefinedSequences = {
    square: [
      { command: "forward", duration: 2000, speed: 2 },
      { command: "right", duration: 500, speed: 2 },
      { command: "forward", duration: 2000, speed: 2 },
      { command: "right", duration: 500, speed: 2 },
      { command: "forward", duration: 2000, speed: 2 },
      { command: "right", duration: 500, speed: 2 },
    ...
    circle: [
      { command: "forward_right", duration: 1000, speed: 2 },
      { command: "forward_right", duration: 1000, speed: 2 },
      { command: "forward_right", duration: 1000, speed: 2 },
      { command: "forward_right", duration: 1000, speed: 2 },
      { command: "forward_right", duration: 1000, speed: 2 },
    ...
    zigzag: [
      { command: "forward", duration: 1000, speed: 2 },
      { command: "right", duration: 300, speed: 2 },
      { command: "forward", duration: 1000, speed: 2 },
      { command: "left", duration: 600, speed: 2 },
      { command: "forward", duration: 1000, speed: 2 },
      { command: "right", duration: 600, speed: 2 },
      { command: "forward", duration: 1000, speed: 2 },
      { command: "left", duration: 300, speed: 2 },
    ],
  };
